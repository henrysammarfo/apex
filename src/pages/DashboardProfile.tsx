...
import { User, Mail, Shield, CheckCircle2, Wallet, Link2, X, Eye, EyeOff, Key, Lock } from 'lucide-react';
      ...
      <AnimatePresence>
        {show2FAModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4" onClick={() => setShow2FAModal(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} onClick={(e) => e.stopPropagation()} className="liquid-glass rounded-2xl w-full max-w-sm border border-border">
              <div className="px-6 py-5 border-b border-border flex items-center justify-between">
                <h2 className="font-inter font-bold text-foreground text-[15px]">Enable 2FA</h2>
              ...
              <div className="p-6 space-y-4">
                <div className="bg-secondary/30 rounded-lg p-4 flex items-center justify-center">
                  <div className="w-32 h-32 bg-foreground/10 rounded-lg flex items-center justify-center">
                    <p className="font-inter text-[11px] text-muted-foreground text-center px-2">QR Code placeholder<br />— connect backend —</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="000000" maxLength={6} className="bg-secondary/50 border-border text-center tracking-[0.5em] font-mono" />
                </div>
                <Button className="w-full font-inter font-bold">Verify & Enable</Button>
              </div>
            </motion.div>
          </motion.div>
        )}
